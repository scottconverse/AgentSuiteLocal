...
UX-1 (post-v0.8.7): installer is 6 steps (Welcome → License → Hardware & model
→ Ollama & model → Smoke test → Ready). The smoke step exercises the real
Python kernel path so missing-bundle bugs surface during install, not on
the user's first New Run.
"""
...
def _enter_app(page: Page, base_url: str) -> None:
    """Walk the 6-step installer to reach the main app."""
    page.goto(base_url)
